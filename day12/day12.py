...
def is_within_bounds(position: Position) -> bool:
	return position[0] >= 0 and position[0] < len(input[0]) and position[1] >= 0 and position[1] < len(input)
...
def expand_region(plant_type: str, region: int, plant: Position):
	for offset in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
		...
		if offsetted in regions[region] or not is_within_bounds(offsetted) or input[offsetted_x][offsetted_y] != plant_type:
			continue
